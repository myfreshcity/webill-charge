...
def update_plan_fee(fee,delay_day,id):
    sql='UPDATE t_contract_repay SET fee = %s,delay_day = %s where id = %s'
    param = (fee,delay_day,id)
    return sql,param

@Insert
def update_plan_by_commit(commit_id):
    sql='UPDATE t_contract_repay SET settled_by_commit = NULL where settled_by_commit = %s'
    param = (commit_id,)
    return sql,param

@Insert
def update_commit(is_valid,is_settled,result,commit_id):
    sql = 'UPDATE t_commit_info SET is_valid = %s,is_settled=%s,result = %s WHERE id = %s'
...
def refund_newest_date():
    sql="SELECT MAX(r.refund_time) AS refund_time, IFNULL(r.bank, r.method) way, ct.shop AS shop FROM t_repayment r LEFT JOIN t_contract ct ON r.contract_id = ct.id GROUP BY r.bank, r.method, ct.shop"
    param = ()
    return sql,param
...
def get_real_pays(contract_id):
    sql="SELECT d.refund_time, d.refund_name, d.amount, IFNULL(d.bank, d.method) way FROM t_repayment d WHERE d.contract_id = %s"
    param=(contract_id,)
    return sql,param
...
def get_refuse_reason(contract_id):
    sql="SELECT t.* FROM(SELECT cr.approve_remark,cr.result FROM t_commit_info cr WHERE cr.contract_id = %s AND cr.result=0 ORDER BY cr.approve_date DESC) t LIMIT 1"
    param=(contract_id,)
    return sql,param
